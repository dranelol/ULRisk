...
public class SerializedDictionary<TKey, TValue> : IDictionary<TKey, TValue>, ICollection<KeyValuePair<TKey, TValue>>, IEnumerable<KeyValuePair<TKey, TValue>>, IDictionary, ICollection, IEnumerable, ISerializable, IDeserializationCallback, ISerializationCallbackReceiver
{
    [SerializeField]
        ...
        if (keys.Count == values.Count)
        {
            ...
            for (int i = 0; i < keys.Count; i++)
            {
                if (keys[i] != null && (!(keys[i] is UnityEngine.Object) || ((UnityEngine.Object)(object)keys[i])))
                {
                    ...
                    if(values.Count <= i)
                    {
                        dictionary.Add(keys[i], default(TValue));
                    }
                    */
